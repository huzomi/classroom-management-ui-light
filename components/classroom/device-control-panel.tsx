        ...
        <div className="rounded-md bg-secondary/50 p-2">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            <ControlButton
              ...
              onClick={() => setDevices((p) => ({ ...p, ac: { ...p.ac, on: true } }))}
              icon={Power}
              label="开"
              variant="primary"
            />
            <ControlButton
              active={!devices.ac.on}
              onClick={() => setDevices((p) => ({ ...p, ac: { ...p.ac, on: false } }))}
              icon={PowerOff}
              label="关"
            />
            <ControlButton
              active={devices.ac.on && devices.ac.mode === "cool"}
              onClick={() => setDevices((p) => ({ ...p, ac: { ...p.ac, on: true, mode: "cool" } }))}
              icon={Snowflake}
              label="制冷"
            />
            <ControlButton
              active={devices.ac.on && devices.ac.mode === "heat"}
              onClick={() => setDevices((p) => ({ ...p, ac: { ...p.ac, on: true, mode: "heat" } }))}
              icon={ThermometerSun}
              label="制热"
            />
          </div>
